"""
signavatars_to_handmdm.py
=========================
Convert SignAvatars 182-dim SMPL-X pose vectors into HandMDM's 274-dim
motion feature format, so HandMDM can be fine-tuned on ASL data.

RUN THE ROUND-TRIP TEST FIRST (--selftest). It takes an existing HandMDM
.npy, decodes it with HandMDM's own np_feats_to_smplx, re-encodes it with
the inverse implemented here, and checks the original is recovered. If that
does not close to ~1e-5, the inverse is wrong and nothing downstream is
trustworthy.

Layouts
-------
SignAvatars 182 (offsets 66:111 / 111:156 confirmed by prep_adapter_data.py,
the rest inferred from per-block value ranges):

      0:3    global_orient    (starts at -pi)
      3:66   body_pose        21 joints x 3, axis-angle
     66:111  left_hand_pose   15 x 3
    111:156  right_hand_pose  15 x 3
    156:179  jaw(3) + expression(10) + betas(10)   [near-static]
    179:182  transl           (the ~19.9 is camera Z)

HandMDM 274 (from prepare/motion_feats_to_smpl.py):

      0:78   body_pose        13 joints x 6D rot
     78:168  left_hand_pose   15 x 6D
    168:258  right_hand_pose  15 x 6D
    258:264  jaw_pose         1 x 6D
    264:274  expression       10

Three transforms, all invertible:
  1. axis-angle <-> rot6d, via HandMDM's own to_matrix / matrix_to
  2. SIGN FLIP -- their decoder negates, so the encoder must negate back
  3. 21 body joints -> 13, dropping body_pose_lower_joints [0,1,3,4,6,7,9,10]

global_orient, betas and transl have no slot in the 274 format and are
dropped. That is fine for fine-tuning: HandMDM never modelled them.

Usage
-----
    python signavatars_to_handmdm.py --selftest --npy <a HandMDM .npy>
    python signavatars_to_handmdm.py --pkl_dir <dir> --out feats274/

Run from the HandMDM directory (needs src.tools.geometry on the path).
"""
import argparse
import glob
import io
